...
,case when capacity <= total then capacity else total end as booked_cnt
,case when capacity <= total then total - capacity else 0 end as waitlist_cnt 
from (
select a.flight_id, a.capacity
,b.passenger_id
,count(b.passenger_id) over (partition by a.flight_id) as total
from flights as a 
