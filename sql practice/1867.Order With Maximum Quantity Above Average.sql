...
,final as (
select 
a.order_id,a.max_quantity,b.max_quantity as target_q
,case when a.max_quantity > b.avg_quantity then 1 else 0 end as flag
from cte as a
cross join cte as b
)
...
from final
group by order_id
having sum(flag) = (select count(distinct order_id) from OrdersDetails)