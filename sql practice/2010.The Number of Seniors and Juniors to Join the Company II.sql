...
with cte as (
    select *
    ,sum(salary) over (partition by experience order by salary) as culmulative_salary
    from candidates
)
...
and culmulative_salary <= 70000
union all
select employee_id
from cte
where experience = 'Junior'
and culmulative_salary <= (select 70000 - ifnull(max(culmulative_salary),0) from cte where experience='Senior' and culmulative_salary <= 70000 )