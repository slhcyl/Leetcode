...
,a.student_id
,a.score
,rank() over (partition by a.exam_id order by a.score desc) as ranknbr
,rank() over (partition by a.exam_id order by a.score) as reverseranknbr
from exam as a 
)
...
select student_id from summary
where ranknbr = 1 or reverseranknbr = 1
)
order by student_id
