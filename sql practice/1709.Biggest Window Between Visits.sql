...
from 
(select user_id,visit_date ,datediff(next_date,visit_date) as biggest_window
from summary) as t 
group by user_id
order by user_id
...
select a.user_id
,a.visit_date
,isnull(lead(visit_date) over (partition by user_id order by visit_date),'2021-1-1') as next_date
from uservisits as a 
)

select user_id, max(biggest_window) as biggest_window
from 
(select user_id,visit_date ,datediff(day,visit_date,next_date) as biggest_window
from summary) as t 
group by user_id
order by user_id
